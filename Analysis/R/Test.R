...
for(x in 1:length(my_data)){
  ...
  # Subset in training
  training[[x]] = filter(df, Id %in% Subjects)
  training[[x]]$Id = factor(training[[x]]$Id)   # making sure they are factors
  training[[x]]$Emotion = factor(training[[x]]$Emotion) # making sure they are factors
...
for(y in 1:length(training)){
  ...
  models[[y]] = glm(Emotion ~ Corr*Zyg, family = binomial(), data= training[[y]])
...
write.csv(comp,file=paste(output_dir,"test.csv", sep  = "\\"))
...
ggsave(paste(output_dir,"Auc_Test.png", sep  = "\\"), width = 20, height = 11.69, units = 'in', dpi=300)
...
write.csv(long,file=paste(output_dir,"test_long.csv", sep  = "\\"))
...
ggsave(paste(output_dir,"Auc_Pcp_Test.png", sep  = "\\"), width = 20, height = 11.69, units = 'in', dpi=300)
