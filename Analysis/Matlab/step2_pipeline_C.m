...
for i=1:numel(data_trl_clean.trial)
  ...
  baseline_corr(i)  = mean(data_trl_clean.trial{i}(1,1:begsample-1));
  baseline_zyg(i)   = mean(data_trl_clean.trial{i}(2,1:begsample-1));
  active_corr(i)    = mean(data_trl_clean.trial{i}(1,begsample:endsample));
  active_zyg(i)     = mean(data_trl_clean.trial{i}(2,begsample:endsample));
end
...
if exist('PipelineC', 'var')
    PipelineC  = [ PipelineC; data_export_not_corrected ];
else
    PipelineC = data_export_not_corrected;
end
...
if exist('PipelineC1', 'var')
    PipelineC1  = [ PipelineC1; d_data_export ];
else
    PipelineC1 = d_data_export;
end
...
if exist('PipelineC2', 'var')
    PipelineC2  = [ PipelineC2; s_data_export ];
else
    PipelineC2 = s_data_export;
end
